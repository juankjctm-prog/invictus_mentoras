...
def parse_recall(text):
    ...
    for match in matches:
        ...
        if 'incluida en el texto' in q_type.lower():
            # Sometimes they say [Opción múltiple — incluida en el texto], we should probably skip or keep it?
            # Let's keep it but just clean the type name
...
def update_data_file(js_path, qa_dir, array_name):
    ...
    for day_idx in range(1, 11):
        ...
        # Parse main text
        main_txt_path = os.path.join(qa_dir, f"Bloque1_{day_str}.txt")
        if os.path.exists(main_txt_path):
            with open(main_txt_path, "r", encoding="utf-8") as f:
                content = f.read()
                
            text_match = re.search(r'--- TEXTO DE LECTURA ---\n(.*?)\n--- FIN TEXTO DE LECTURA ---', content, re.DOTALL)
            if text_match:
                if (day_idx - 1) < len(data):
                    # Replace only the text part, keep the rest like 'comprension'
                    if 'fase2_lectura' not in data[day_idx - 1]:
                        data[day_idx - 1]['fase2_lectura'] = {}
                    # Prepend (12 min) etc from the original if needed, or just replace entirely.
                    # Usually the text includes the minute marker, or it's added by the UI. Let's just use the raw text.
                    # Wait, the original JS has "(12 min)\nCronómetro activo..." at the beginning. 
                    # The text match usually doesn't have it, or maybe it does? Let's prepend it.
                    orig_text = data[day_idx - 1].get('fase2_lectura', {}).get('texto', '')
                    prefix = ""
                    if orig_text.startswith("(12 min)"):
                        prefix_match = re.match(r'(\(12 min\).*?PPM[^\n]*\n)', orig_text, re.DOTALL)
                        if prefix_match:
        ...
        # Parse Q&A
        qa_path = os.path.join(qa_dir, f"Bloque1_{day_str}_Preguntas_y_Respuestas.txt")
