...
function populatePlaybookDays() {
    ...
    // We populate 78 days
    for(let i=1; i<=78; i++) {
        selector.innerHTML += `<option value="${i}">Día ${i}</option>`;
    }
    
    // Auto select the current active day from UI
    if(window.currentDayIndex) {
...
content = content.replace("</body>", playbook_js)
